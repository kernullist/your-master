...
const tools: Promise<Tool>[] = [
  tool({
    name: 'daily_briefing',
    description: 'Gather the user\'s day-at-a-glance: current date/time, pending to-dos, standing instructions, and upcoming reminders/timers (next 24h). Use for "what\'s my day look like?" or a morning briefing. After calling this, optionally add weather (get_weather) and headlines (web search), then summarize it warmly in chat.',
    execute: async () => {
      const now = Date.now()
      ...
      // Standing instructions (recurring requests) complement one-off todos.
      const standingInstructions = memoryStore.list(characterId)
